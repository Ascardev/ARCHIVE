...
static short	AgingBlinkColor[CONST_AGINGBCOLOR_MAX][6] =
{
	...
	{ 130,	  0,	255,	0,	 0,	smTEXSTATE_FS_SCROLL10	}, 
...
int sinThrowItemToFeild(sITEM* pItem)
{
	...
	if (cShop.OpenFlag || SkillMasterFlag || cCraftItem.OpenFlag || cAging.OpenFlag  || cRemodel.OpenFlag) {
...
int AddExp(INT64 Exp)
{
	...
	if (exp64 > ExpLevelTable[99] && lpCurPlayer->smCharInfo.Level >= 140) {
		CodeXorCharInfo_Exp();
		return TRUE;
	}
...
int QuitGame()
{
	...
	if (cWareHouse.OpenFlag || cCraftItem.OpenFlag || cTrade.OpenFlag || cAging.OpenFlag  || cRemodel.OpenFlag || CoinShop::GetInstance()->IsOpen() || TimeShop::GetInstance()->IsOpen() || UpdateItem::GetInstance()->IsOpen())
	{
