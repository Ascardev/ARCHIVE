...
void cINVENTORY::RButtonDown(int x, int y)
{
	...
	char szBuff[128];
	if (cWareHouse.OpenFlag || cTrade.OpenFlag || cCraftItem.OpenFlag || cShop.OpenFlag  ||
		cMyShop.OpenFlag || cCharShop.OpenFlag )return;
	UsePotion(x, y);
...
int cINVENTORY::NoSettingThrowItem(sITEM* pItem, int Flag)
{
	...
	if (!pItem->Flag)return FALSE;
	if (!Flag) {
		if (cTrade.OpenFlag || cWareHouse.OpenFlag || cCraftItem.OpenFlag || cAging.OpenFlag   || CoinShop::GetInstance()->IsOpen())return FALSE;
	}
...
int cINVENTORY::ThrowInvenItemToField(sITEM* pItem)
{
	...
	if (cTrade.OpenFlag || cWareHouse.OpenFlag || cCraftItem.OpenFlag || cAging.OpenFlag   || cRemodel.OpenFlag || LockItem::GetInstance()->IsOpen())
		return TRUE;
...
int cINVENTORY::SetItemToChar(int CheckHackFlag)
{
	...
	SetBonus1 = (LValRing + RValRing + KelAmulet);
	SetBonus2 = (LShyRing + RShyRing + TullaAmulet + FurySheltom);
	...
	for (i = 0; i < 20; i++) {
		if (ContinueSkill[i].Flag) {
			...
			case SKILL_KRISHNA:
				//mpSkillData = ((float)Krishna_Defense[ContinueSkill[i].Point - 1] / 100);
			//inChar->Defence += (int)((float)sinChar->Defence * TempSkillData);
				sinDefense += (Krishna_Defense[ContinueSkill[i].Point - 1] * (TempDefense)) / 100;
				SkillTextColor[SIN2_DEFENSE] = SIN_TEXT_COLOR_BLUE;
				sinTempAbsorb += Krishna_Abs[ContinueSkill[i].Point - 1];
				SkillTextColor[SIN2_ABSORB] = SIN_TEXT_COLOR_BLUE;
			 //	sinTempAbsorb += sinChar->Absorption * Krishna_Abs[ContinueSkill[i].Point - 1] ;
				//SkillTextColor[SIN2_DEFENSE] = SIN_TEXT_COLOR_BLUE;
			//	SkillTextColor[SIN2_ABSORB] = SIN_TEXT_COLOR_BLUE;
				break;
	...
	for (i = 1; i < MAX_USESKILL; i++) {
		if (sinSkill.UseSkill[i].Flag && sinSkill.UseSkill[i].Point) {
			...
			case SKILL_S_MASTERY:
				...
				SkillTextColor[SIN2_DAMAGE] = SIN_TEXT_COLOR_GOLD;
				break;
			case SKILL_D_MASTERY2:
				sinTempDamage2[0] += (int)(sinChar->Attack_Damage[0] * D_Mastery2_Damage[sinSkill.UseSkill[i].Point - 1]) / 100;
				sinTempDamage2[1] += (int)(sinChar->Attack_Damage[1] * D_Mastery2_Damage[sinSkill.UseSkill[i].Point - 1]) / 100;
				SkillTextColor[SIN2_DAMAGE] = SIN_TEXT_COLOR_GOLD;
				break;
			}
		}
